...
namespace Rails
{
	...
	public class TopCommodities : System.Windows.Forms.Form
	{
		...
		public TopCommodities(Form owner, Game game, Map map)
		{
			...
			Progress pr = new Progress("Computing average payoff amounts");
			...
			for (int i=0; i<n; i++)
			{
				...
				int[] count = new int[3];
				map.ResetFloodMap();
				foreach (int source in map.ProductSources[i])
					map.SetValue(map.Cities[source].X, map.Cities[source].Y, 0);
				map.Flood(map.cityDistanceMethod);
				for (int j=0; j<map.CityCount; j++)
				{
					int dist = map[map.Cities[j].X, map.Cities[j].Y].Value;
					if (dist > 0 && dist < int.MaxValue)
					{
						int kind;
						if (j < map.NumCapitals)
							kind = 0;
						else if (j < map.NumCapitals + map.NumCities)
							kind = 1;
						else
							kind = 2;
						total[kind] += dist;
				...
				score[i] = (total[0] / count[0] + total[1] / count[1] + total[2] / count[2]) / 3.0;
				pr.SetProgress((i+1)*100/n);
			}
			pr.Close();
			pr.Dispose();
